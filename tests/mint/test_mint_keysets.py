...
short_keys = {k: PublicKey(bytes.fromhex(v), raw=True) for k, v in short_keys.items()}
...
keys = {k: PublicKey(bytes.fromhex(v), raw=True) for k, v in keys.items()}

def test_keyset_id_derivation_v1():
    assert derive_keyset_id(short_keys) == "00456a94ab4e1c46"
    assert derive_keyset_id(keys) == "000f01df73ea149a"

def test_keyset_id_derivation_v2():
    unit = "sat"
    expiry = 2059210353
    assert derive_keyset_id_v2(short_keys, unit, expiry) == "01adc013fa9d85171586660abab27579888611659d357bc86bc09cb26eee8bc035"
    assert derive_keyset_id_v2(keys, unit, expiry) == "0125bc634e270ad7e937af5b957f8396bb627d73f6e1fd2ffe4294c26b57daf9e0"
